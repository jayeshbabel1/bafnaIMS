...
$specs = [
     ui('filter_stone_type', 'Stone Type')     => tr('product',$id,'category',$p['category']),
    ui('spec_subcategory', 'Subcategory')    => tr('product',$id,'subcategory',$p['subcategory']),
    ui('filter_color', 'Color')         => tr('product',$id,'color_subcategory',$p['color_subcategory']),
    ui('spec_quarry_no', 'Quarry No.')     => $p['quarry_number'], // no translate (value)
    ui('spec_total_pieces', 'Total Pieces')   => $p['pieces'] ? $p['pieces'].' '.ui('unit_slabs', 'slabs') : '',
    ui('label_thickness', 'Thickness')      => $p['thickness'], // no translate (measurement)
    ui('label_useable_size', 'Useable Size')   => $sizesDisplay,
    ui('label_italian_size', 'Italian Size')   => $cutterDisplay,
    ui('spec_origin', 'Origin')         => tr('product',$id,'origin',$p['origin']),
    ui('spec_finish', 'Finish')         => tr('product',$id,'finish',$p['finish']),
];
?>
...
<div class="detail-page">
  ...
  <div class="detail-hero" id="heroWrap">
    ...
    <div class="detail-hero-top">
      ...
      <div class="detail-hero-actions">
        ...
        <a href="assets/uploads/photos/<?= h($photos[0]['filename']) ?>"
           ...
           class="hero-icon-btn" id="heroDownloadBtn" title="<?= h(ui('title_download_image', 'Download image')) ?>">
          <?= icon('download',16) ?>
        </a>
        <?php endif; ?>
    ...
    <div class="zoom-controls zoom-controls--hero" data-target="hero">
      <button class="zoom-btn" data-action="in" title="<?= h(ui('title_zoom_in', 'Zoom in')) ?>"><?= icon('zoom',14) ?></button>
      <button class="zoom-btn" data-action="out" title="<?= h(ui('title_zoom_out', 'Zoom out')) ?>"><?= icon('search',14) ?></button>
    ...
    <div class="detail-status">
     <?= ($p['in_stock'] && (float)$p['quantity_available'] > 0) ? '<span class="badge badge-green">● '.h(ui('label_in_stock', 'In Stock')).'</span>' : '<span class="badge badge-gray">'.h(ui('label_out_of_stock', 'Out of Stock')).'</span>' ?>
      <?php if ($p['featured']): ?><span class="badge badge-gold">★ <?= h(ui('badge_featured', 'Featured')) ?></span><?php endif; ?>
    </div>
  ...
  <div class="detail-body">
    ...
    <p class="detail-quarry"><?= h(ui('spec_quarry_no', 'Quarry No.')) ?> <?= h($p['quarry_number']) ?><?= $p['origin'] ? ' · '.h($p['origin']) : '' ?></p>
    ...
    <div class="qty-strip">
      <div class="qty-tile">
        <div class="qty-tile-label"><?= h(ui('qty_tile_total', 'Total')) ?></div>
        <div class="qty-tile-value"><?= number_format((float)$p['total_quantity']) ?></div>
        <div class="qty-tile-unit"><?= h(ui('unit_sqft', 'sqft')) ?></div>
      </div>
      <div class="qty-tile">
        <div class="qty-tile-label"><?= h(ui('qty_tile_on_hold', 'On Hold')) ?></div>
        <div class="qty-tile-value"><?= number_format((float)$p['quantity_on_hold']) ?></div>
        <div class="qty-tile-unit"><?= h(ui('unit_sqft', 'sqft')) ?></div>
      </div>
      <div class="qty-tile" style="border:1px solid var(--black);">
        <div class="qty-tile-label"><?= h(ui('qty_tile_available', 'Available')) ?></div>
    ...
    <div class="spec-grid">
      <?php
      $quickSpecs = [ui('label_thickness','Thickness')=>$p['thickness'], ui('spec_finish','Finish')=>$p['finish'], ui('label_useable_size','Useable Size')=>$sizesDisplay, ui('label_italian_size','Italian Size')=>$cutterDisplay];
    ...
    <div class="spec-table-card">
      <div class="spec-table-header"><?= h(ui('section_full_specifications', 'Full Specifications')) ?></div>
    ...
    <p class="section-label"><?= h(ui('section_gallery', 'Gallery')) ?></p>
...
<p class="section-label"><?= h(ui('section_video', 'Video')) ?></p>
...
<a href="https://wa.me/?text=<?= $vidShareMsg ?>" target="_blank" rel="noopener"
  ...
  <?= icon('whatsapp',15) ?> <?= h(ui('btn_share_video_whatsapp', 'Share Video on WhatsApp')) ?>
</a>
</div>
<?php endif; ?>   
    ...
    <p class="section-label"><?= h(ui('section_documents', 'Documents')) ?></p>
    <div class="doc-card">
      <div class="doc-icon"><?= icon('file',20) ?></div>
      <div class="doc-info">
        <p class="doc-name"><?= h(ui('doc_product_pdf', 'Product PDF')) ?></p>
        <p class="doc-meta"><?= h(ui('doc_product_detail_pdf', 'Product Detail PDF')) ?></p>
      </div>
    ...
    <div class="doc-card">
      ...
      <div class="doc-info">
        ...
        <p class="doc-meta"><?= h(ui('doc_measurement_sheet', 'Measurement Sheet')) ?></p>
      </div>
    ...
    <div class="doc-card">
      ...
      <div class="doc-info">
        ...
        <p class="doc-meta"><?= h(ui('doc_dna_report', 'DNA Report')) ?></p>
      </div>
    ...
    <div class="detail-cta" style="margin-top:22px;">
      <form method="POST" action="index.php" style="flex:1">
        ...
        <button type="submit" class="btn btn-secondary btn-block">
          <?= $saved ? icon('heart_fill',16).'&nbsp;'.h(ui('btn_saved', 'Saved')) : icon('heart',16).'&nbsp;'.h(ui('btn_save_to_shortlist', 'Save to Shortlist')) ?>
        </button>
      </form>
      <button type="button" onclick="openWaPdfShare()" class="btn btn-secondary" style="flex:1;">
        <?= icon('whatsapp',16) ?>&nbsp; <?= h(ui('btn_whatsapp_share', 'WhatsApp Share')) ?>
      </button>
    ...
    <div style="margin-top:10px;">
      <button onclick="openAddToSelection()" class="btn btn-gold btn-block btn-lg">
        <?= icon('plus',16) ?>&nbsp; <?= h(ui('btn_add_to_client_selection', 'Add to Client Selection')) ?>
      </button>
    </div>
    ...
    <div style="margin-top:10px;">
      <button type="button" class="btn btn-secondary btn-block btn-lg"
        ...
        <?= icon('grid',16) ?>&nbsp; <?= h(ui('btn_slab_calculator', 'Slab Calculator')) ?>
      </button>
    </div>
  ...
  <a href="index.php?page=room_visualizer&product_id=<?= $id ?>" class="btn btn-secondary btn-block btn-lg" style="text-decoration:none;">
    <?= icon('image',16) ?>&nbsp; <?= h(ui('btn_visualize_in_room', 'Visualize in a Room')) ?>
  </a>
</div>
<div style="margin-top:10px;">
  <button type="button" class="btn btn-secondary btn-block btn-lg" onclick="open3DPreview()">
    <?= icon('grid',16) ?>&nbsp; <?= h(ui('btn_3d_room_preview', '3D Room Preview')) ?>
  </button>
</div>
...
<div id="rv3dModal" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,.85);z-index:9500;align-items:center;justify-content:center;padding:14px;">
  <div style="background:var(--white);border-radius:var(--radius-xl);max-width:1080px;width:100%;max-height:92vh;overflow:hidden;display:flex;flex-direction:column;">
    <div style="display:flex;align-items:center;justify-content:space-between;padding:14px 18px;border-bottom:1px solid var(--border);flex-shrink:0;position:relative;z-index:2;background:var(--white);">
      <p style="font-weight:700;"><?= h(ui('btn_3d_room_preview', '3D Room Preview')) ?></p>
...
function close3DPreview() { document.getElementById('rv3dModal').style.display = 'none'; }
    ...
    <p style="text-align:center;font-size:12px;color:var(--text4);margin-top:8px;">
      <?= h(ui('empty_no_clients', 'No clients yet')) ?> — <a href="index.php?page=client_form" style="color:var(--black);font-weight:600;"><?= h(ui('link_add_a_client', 'add a client')) ?></a> <?= h(ui('msg_to_use_selections', 'to use selections.')) ?>
    </p>
    <?php endif; ?>
...
<div class="modal-overlay" id="shareModal" onclick="if(event.target===this)closeShareModal()">
  <div class="modal-sheet">
    ...
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:18px;">
      <p style="font-family:var(--font-display);font-size:17px;font-weight:700;"><?= h(ui('title_share_product', 'Share Product')) ?></p>
    ...
    <a href="https://wa.me/?text=<?= $shareText ?>%20<?= urlencode($shareUrl) ?>" target="_blank" class="share-option">
      <div class="share-icon" style="color:#25D366;"><?= icon('whatsapp',20) ?></div><span><?= h(ui('btn_share_whatsapp', 'Share via WhatsApp')) ?></span>
    </a>
    <a href="mailto:?subject=<?= $shareText ?>&body=<?= urlencode($shareUrl) ?>" class="share-option">
      <div class="share-icon" style="color:var(--gold);"><?= icon('mail',20) ?></div><span><?= h(ui('btn_share_email', 'Share via Email')) ?></span>
    </a>
    <button class="share-option" onclick="copyLink('<?= h($shareUrl) ?>')">
      <div class="share-icon"><?= icon('copy',20) ?></div><span id="copyLinkLabel"><?= h(ui('btn_copy_link', 'Copy Link')) ?></span>
    </button>
  </div>
  <?= roomAreaDatalist() ?>
</div>
...
<div id="addToSelModal" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,.6);z-index:9000;align-items:flex-end;justify-content:center;">
  <div style="background:var(--white);border-radius:var(--radius-xl) var(--radius-xl) 0 0;width:100%;max-width:100%;max-height:92vh;overflow-y:auto;">
    ...
    <div style="padding:14px 20px;border-bottom:1px solid var(--border);display:flex;align-items:center;justify-content:space-between;position:sticky;top:0;background:var(--white);z-index:2;">
      <p style="font-family:var(--font-display);font-size:16px;font-weight:700;"><?= h(ui('btn_add_to_client_selection', 'Add to Client Selection')) ?></p>
    ...
    <div style="padding:14px 20px;background:var(--gray-50);border-bottom:1px solid var(--border);">
      <div style="display:flex;gap:12px;align-items:center;">
        ...
        <div style="flex:1;min-width:0;">
          ...
          <p style="font-size:12px;color:var(--text3);margin-top:1px;"><?= h(ui('label_lot', 'Lot')) ?> <?= h($p['quarry_number']) ?></p>
          <div style="display:flex;gap:4px;flex-wrap:wrap;margin-top:4px;">
            <?php if ($p['thickness']): ?>
            <span class="badge badge-gray" style="font-size:10px;"><?= h($p['thickness']) ?></span>
            <?php endif; ?>
            <?php if ($p['quantity_available']): ?>
            <span class="badge badge-green" style="font-size:10px;"><?= number_format((float)$p['quantity_available']) ?> <?= h(ui('unit_sqft_avail', 'sqft avail.')) ?></span>
            <?php endif; ?>
            <?php if ($sizesDisplay): ?>
            <span class="badge badge-white" style="font-size:10px;"><?= h(ui('label_size', 'Size:')) ?> <?= h($sizesDisplay) ?></span>
            <?php endif; ?>
          </div>
        </div>
      </div>
    ...
    <div style="padding:20px;">
      ...
      <div style="text-align:center;padding:20px 0;">
        ...
        <p style="font-weight:700;font-size:15px;margin-bottom:6px;"><?= h(ui('empty_no_clients', 'No clients yet')) ?></p>
        <p style="font-size:13px;color:var(--text3);margin-bottom:18px;"><?= h(ui('msg_add_client_first', 'Add a client first to save product selections for them.')) ?></p>
      ...
      <form method="POST" action="index.php" id="addToSelForm">
        ...
        <div class="input-group">
          <label class="input-label"><?= h(ui('form_client', 'Client')) ?> <span style="color:var(--danger);">*</span></label>
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;">
          <div class="input-group">
            <label class="input-label"><?= h(ui('form_area_room', 'Area / Room')) ?></label>
       ...
       placeholder="<?= h(ui('form_area_room_placeholder', 'e.g. Master Bedroom')) ?>" list="roomAreaSuggestions" autocomplete="off"/>
          ...
          <div class="input-group">
            <label class="input-label"><?= h(ui('form_qty_required', 'Qty Required (sqft)')) ?></label>
